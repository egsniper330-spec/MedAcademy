-- v212 — Fix hard_delete_user(): remove invalid 'deleted' from course_status comparison
--
-- ROOT CAUSE:
--   hard_delete_user() guards against deleting a doctor with active courses via:
--     WHERE doctor_id = p_target_user_id AND status NOT IN ('archived','deleted')
--   But courses.status is type course_status ENUM ('draft','published','hidden','archived').
--   'deleted' is NOT a valid course_status value → Postgres throws:
--     "invalid input value for enum course_status: deleted"
--   This aborts the entire RPC → every permanent user deletion fails.
--
-- WHY 'deleted' SHOULD NOT BE IN course_status:
--   Courses are HARD DELETED (DELETE FROM courses). Dead rows don't exist.
--   The courses.permanently_deleted boolean exists for soft tracking.
--   The correct check is: count courses that are still live (not yet archived).
--
-- FIX: Replace NOT IN ('archived','deleted') with != 'archived'
--   All non-archived course statuses (draft, published, hidden) count as active.
--   Hard-deleted courses are gone from the table, so they can never be counted.
...
BEGIN
  ...
  -- Courses are hard-deleted; use != 'archived' to count all live courses.
...
-- Sanity check: verify 'deleted' is not in course_status (should return 0 rows)
SELECT enumlabel
FROM   pg_enum
WHERE  enumtypid = 'public.course_status'::regtype
