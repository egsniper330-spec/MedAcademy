...
export function LargeFileWarning({ visible, analysis, fileName, onProceed, onCancel }: Props) {
  ...
  // Responsive card width: fills available width with consistent horizontal margins
  const cardW = Math.min(screenW - Math.max(40, insets.left + insets.right + 40), 420);
  // Cap height so buttons remain accessible at large accessibility font sizes
  ...
  return (
    <Modal visible={visible} transparent animationType="fade" statusBarTranslucent>
      <View style={{
        ...
        <NeuCard style={[neuFlatStyle(isDark), {
          borderRadius: 20,
          width: cardW,
          maxHeight: cardMaxH,
          overflow: 'hidden',
