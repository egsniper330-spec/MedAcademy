/**
 * TextEncoder / TextDecoder polyfill for JSC (iOS) — SCRIPT CONTEXT SAFE.
 *
 * WHY THIS EXISTS
 * ---------------
 ...
 *
 * WHY require() CANNOT BE USED HERE
 * ----------------------------------
 * This file is injected via metro.config.js `serializer.polyfillModuleNames`.
 * Metro transforms polyfillModuleNames with type:"script" (getPrependedScripts.js
 * line 54). Scripts are emitted as RAW code — no __d() CJS wrapper, no require()
 * function in scope. Using require() here would crash with:
 *   ReferenceError: Can't find variable: require
 * at global code@<line>:<col> — exactly the device crash we are fixing.
 *
 * FIX
 * ---
 * The content of fast-text-encoding/text.min.js is inlined directly here.
 * It is a self-contained IIFE with zero require() calls — safe in script context.
 * Scope selection: typeof window !== 'undefined' ? window :
 *                  typeof global !== 'undefined' ? global : this
 * On JSC: window=undefined, global=<globalObject> → installs on global. ✓
 * On Hermes/browser: TextEncoder already present → OR-guard is a no-op. ✓
 *
 * SOURCE: fast-text-encoding@1.0.6 — Apache-2.0 license
 * https://github.com/samthor/fast-text-encoding
 */

// ↓ Inlined fast-text-encoding@1.0.6 text.min.js — NO require(), script-safe.
// jscs:disable
/* eslint-disable */
