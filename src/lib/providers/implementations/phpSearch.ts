/**
 * implementations/phpSearch.ts
 * PHP-backed search implementation of SearchProvider.
 */
import { backendClient } from '@/client/backendClient';
import type { SearchProvider, SearchQuery, SearchResponse, SearchResult } from '../searchProvider';

// Map index names to PHP-backed tables and searchable columns
...
class PhpSearchProvider implements SearchProvider {
  readonly providerKey = 'php_search';
  readonly displayName = 'PHP Search';
  ...
  async indexDocument(_index: string, _id: string, _document: Record<string, unknown>): Promise<void> {
    // The PHP data layer handles persistence; no client-side index management is needed
  }
  ...
  async checkHealth(): Promise<'healthy' | 'warning' | 'offline' | 'maintenance' | 'unknown'> {
    try {
      await backendClient.from('courses').select('id').limit(1);
      return 'healthy';
    } catch {
      return 'offline';
    }
...
export const phpSearchProvider = new PhpSearchProvider();
